...
const RegimeComparison = React.memo(({ hideHero = false }: { hideHero?: boolean }) => {
  ...
  return (
    <div className="flex flex-col gap-6 w-full text-left">
      ...
      {!hideHero && (
        <>
          ...
          <motion.div 
            ...
            className="bg-slate-50 dark:bg-[#0E131B] border border-slate-200 dark:border-white/[0.04] rounded-3xl p-6 lg:p-8 relative overflow-hidden shadow-sm dark:shadow-2xl space-y-6"
          >
            ...
            <div className="space-y-1 text-left">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest font-black">Estimated Tax Savings</span>
              <div className="text-5xl lg:text-6xl font-black text-emerald-600 dark:text-emerald-400 font-mono tracking-tighter leading-none select-all">
                <SavingsCounter value={savings} />
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-455 font-bold uppercase tracking-wider pt-1">
                Compared with your current filing strategy
              </p>
            </div>

            <div className="h-px bg-slate-200 dark:bg-white/[0.04] w-full" />
            ...
            <div className="space-y-2.5 text-left select-none">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest font-black block">Verified against</span>
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <div key={check} className="flex items-center gap-1.5 bg-blue-50 dark:bg-blue-600/5 border border-blue-200 dark:border-blue-500/10 px-3 py-1 rounded-lg text-[10px] text-blue-600 dark:text-blue-400 font-bold">
      ...
      <div className="flex items-center gap-1 p-1 bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-white/[0.04] rounded-xl self-start mt-2 select-none">
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="bg-slate-50 dark:bg-[#0E131B] border border-slate-205 dark:border-white/[0.04] rounded-2xl p-5 space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-1 text-[9px] font-bold text-emerald-600 dark:text-emerald-450 uppercase bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 px-2 py-0.5 rounded-full">
              ...
              <div className="bg-slate-50 dark:bg-[#0E131B] border border-slate-205 dark:border-white/[0.04] rounded-2xl p-5 space-y-3.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 text-[10px] font-bold text-slate-650 dark:text-slate-400 select-none">
                  {[
                    ...
                    <div key={step} className="flex items-center gap-2 bg-white dark:bg-slate-950/40 p-2.5 rounded-xl border border-slate-200 dark:border-white/[0.02]">
                      <span className="w-4 h-4 rounded-full bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-450 flex items-center justify-center font-mono text-[8.5px]">✓</span>
                      <span>{step}</span>
                    </div>
                  ))}
                </div>
              </div>              {/* Expandable What Changed */}
              <div className="border border-slate-200 dark:border-white/[0.04] rounded-2xl overflow-hidden">
                <button
                  onClick={() => setIsWhatChangedOpen(!isWhatChangedOpen)}
                  className="w-full p-4 flex justify-between items-center bg-slate-50/50 hover:bg-slate-50 dark:bg-[#0E131B]/50 dark:hover:bg-[#0E131B] transition-colors cursor-pointer text-left border-none"
                >
                  <span className="text-[10px] text-slate-600 dark:text-slate-350 uppercase tracking-widest font-black">Compared to your current filing</span>
                  {isWhatChangedOpen ? <ChevronUp className="h-4 w-4 text-slate-500 dark:text-slate-455" /> : <ChevronDown className="h-4 w-4 text-slate-500 dark:text-slate-455" />}
                </button>
                ...
                <AnimatePresence>
                  {isWhatChangedOpen && (
                    <motion.div
                      ...
                      animate={{ height: 'auto' }}
                      exit={{ height: 0 }}
                      className="overflow-hidden bg-slate-50/30 dark:bg-[#0D1117]/30 border-t border-slate-200 dark:border-white/[0.02]"
                    >
                      <div className="p-4 space-y-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
                        ...
                        <div className="flex justify-between items-center py-1 border-t border-slate-200 dark:border-white/[0.02] pt-2">
                          ...
                          <span className="text-slate-700 dark:text-slate-300 font-mono">- Lower taxable base</span>
                        </div>
                        <div className="flex justify-between items-center py-1 border-t border-slate-200 dark:border-white/[0.02] pt-2">
                          <span>Slabs configuration:</span>
                          <span className="text-slate-700 dark:text-slate-300">✓ Better slab selection</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
                ...
                <div className="bg-slate-50 dark:bg-[#0E131B] border border-slate-205 dark:border-white/[0.04] rounded-2xl p-5 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5 select-none">
                    {[
                      ...
                      <span key={tag} className="px-2.5 py-1 bg-emerald-500/5 border border-emerald-200 dark:border-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold rounded-lg">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                <div className="bg-slate-50 dark:bg-[#0E131B] border border-slate-205 dark:border-white/[0.04] rounded-2xl p-5 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-1.5 select-none">
                    {[
                      ...
                      <span key={tag} className="px-2.5 py-1 bg-slate-100/50 dark:bg-slate-900/60 border border-slate-200 dark:border-white/[0.02] text-slate-600 dark:text-slate-400 text-[10px] font-bold rounded-lg">
                        {tag}
                      </span>
                    ))}
                  </div>
          ...
          {activeSection === 'details' && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              className="bg-slate-50 dark:bg-[#0E131B] border border-slate-200 dark:border-white/[0.04] rounded-2xl p-5 shadow-sm"
            >
              <div className="border border-slate-200 dark:border-white/[0.03] rounded-xl overflow-hidden bg-white dark:bg-slate-950/40">
                <table className="w-full text-xs text-left">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-white/[0.04] bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-500 font-semibold font-mono">
                      <th className="p-3">Tax Parameter</th>
                      <th className="p-3 text-right">Old Regime</th>
                      <th className="p-3 text-right text-blue-600 dark:text-blue-400">New Regime</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-white/[0.02] text-slate-600 dark:text-slate-350 font-medium">
                    <tr>
                    ...
                    <tr>
                      ...
                      <td className="p-3 text-right font-mono text-emerald-600 dark:text-emerald-450">-{formatINR(oldRegime.rebate87A)}</td>
                      <td className="p-3 text-right font-mono text-emerald-600 dark:text-emerald-450">-{formatINR(newRegime.rebate87A)}</td>
                    </tr>
                    <tr>
                    ...
                    <tr className="bg-slate-100/50 dark:bg-slate-900/50">
                      <td className="p-3 font-extrabold text-slate-900 dark:text-slate-100">Total Tax Payable</td>
                      <td className="p-3 text-right font-mono font-extrabold text-slate-900 dark:text-slate-100">{formatINR(oldRegime.totalTaxPayable)}</td>
                      <td className="p-3 text-right font-mono font-extrabold text-emerald-600 dark:text-emerald-400">{formatINR(newRegime.totalTaxPayable)}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          {activeSection === 'legal' && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -10 }}
              className="bg-slate-50 dark:bg-[#0E131B] border border-slate-200 dark:border-white/[0.04] rounded-2xl p-5 space-y-4"
            >
              ...
              <ul className="space-y-4 text-xs text-slate-600 dark:text-slate-400 leading-relaxed font-semibold">
                <li className="flex gap-3">
                  ...
                  <div>
                    <strong className="text-slate-800 dark:text-slate-300 block mb-1">Section 115BAC (New Tax Regime)</strong>
                ...
                <li className="flex gap-3">
                  <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
                  <div>
                    <strong className="text-slate-800 dark:text-slate-300 block mb-1">Section 87A Rebate</strong>
