...
export default function ExtractionConfirm({
  ...
  if (isProcessing) {
    return (
      <div id="extraction-loading-state" className="flex flex-col items-center justify-center p-12 bg-transparent text-center space-y-4">
        <RefreshCw className="h-10 w-10 text-emerald-400 animate-spin mb-4" />
        <h3 className="text-base font-bold text-slate-100">Analyzing Form 16 Structure</h3>
        <p className="text-xs text-slate-450 max-w-sm font-medium leading-relaxed">
  ...
  return (
    <div id="extraction-confirm-card" className="bg-transparent text-slate-100 space-y-6 animate-fade-in font-sans">
      {/* Header Info Block */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-white/[0.06] pb-5 mb-6 gap-3">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-white/[0.03] border border-white/[0.08] text-slate-300 rounded-xl">
            <FileText className="h-6 w-6" />
          </div>
          <div className="space-y-0.5 text-left">
            <h3 className="text-base font-bold text-white">Form 16 Extraction Verification</h3>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Verify values extracted by Gemini AI. Keep editing any fields to correct errors before starting.
            </p>
          </div>
        </div>
        <span className="text-xs font-mono px-3.5 py-1 bg-white/[0.03] border border-white/[0.08] text-slate-300 font-bold rounded-full">
      ...
      <div className="space-y-5">
        <h4 className="text-[9px] font-bold text-slate-500 uppercase tracking-widest border-b border-white/[0.04] pb-1.5 mb-2 text-left">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center border-b border-white/[0.04] pb-4">
          <div className="md:col-span-5 space-y-1 text-left">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-slate-300">Employer Name</span>
            </div>
            <p className="text-[10px] leading-relaxed text-slate-500">Company name listed on your Form 16 Annexure</p>
            <div className="flex items-center gap-1.5 mt-1">
              ...
              {extractedData?.employerName ? (
                <span className="text-[9px] font-bold text-emerald-400 bg-emerald-500/5 px-2 py-0.5 rounded border border-emerald-500/10">
                  "{extractedData.employerName}"
                </span>
              ) : (
          ...
          <div className="md:col-span-7">
            <input
              ...
              className={`w-full bg-slate-950/60 border rounded-xl py-2 px-3.5 text-sm text-white focus:outline-none focus:bg-slate-950 transition-all ${
                isEmployerNameEmpty ? 'border-amber-500/30 bg-amber-500/[0.02]' : 'border-white/[0.08] focus:border-emerald-500/50'
              }`}
              placeholder="Enter employer company name"
            />
            {isEmployerNameEmpty && (
              <span className="inline-flex items-center gap-1 text-[9px] font-bold text-amber-400 bg-amber-500/5 border border-amber-500/10 px-2 py-0.5 rounded mt-1.5">
          ...
          <div className="md:col-span-7">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-950/60 border border-white/[0.08] focus:border-emerald-500/50 focus:bg-slate-950 rounded-xl py-2 pl-8 pr-3.5 text-sm text-white font-mono transition-all"
                placeholder="Enter provident fund amount"
              />
            </div>
            {isPfContributionEmpty && (
              <span className="inline-flex items-center gap-1 text-[9px] font-semibold text-slate-500 bg-white/[0.02] border border-white/[0.04] px-2 py-0.5 rounded mt-1.5">
                ⚠️ Action Required: Please fill manually
              </span>
            )}
          </div>
        ...
        <div className="bg-slate-955/40 p-5 rounded-2xl border border-white/[0.06] space-y-4 text-left">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Other Income</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-950 border border-white/[0.08] rounded-lg py-1.5 pl-6 pr-2.5 text-xs text-white font-mono focus:outline-none focus:border-emerald-500/50 focus:bg-slate-950"
                  placeholder="0"
                />
              </div>
            </div>
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Deduction 80C</label>
              <div className="relative">
                <span className="absolute left-3 top-2 text-slate-500 text-[11px] font-semibold">₹</span>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Deduction 80D</label>
              <div className="relative">
                <span className="absolute left-3 top-2 text-slate-500 text-[11px] font-semibold">₹</span>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-1">Basic Salary</label>
              <div className="relative">
                <span className="absolute left-3 top-2 text-slate-500 text-[11px] font-semibold">₹</span>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-[11px] font-bold text-slate-400 mb-1">NPS (80CCD(1B))</label>
              <div className="relative">
                <span className="absolute left-3 top-2 text-slate-500 text-[11px] font-semibold">₹</span>
                <input
                  type="text"
      ...
      <div className="bg-slate-950/40 text-white rounded-2xl p-5 border border-white/[0.06] shadow-md mt-6 text-left">
        <div className="flex items-center gap-2 mb-3">
          <TrendingUp className="h-4.5 w-4.5 text-emerald-450" />
          <h4 className="text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">Live Mathematical Verification Snapshot</h4>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Old Regime Card */}
          <div className="bg-slate-900/60 border border-white/[0.04] rounded-xl p-3.5 flex flex-col justify-between">
            <div>
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Old Tax Regime</span>
              <div className="text-base font-black text-white mt-1 font-mono">
                {formatINR(netTaxableSalaryOld)}
              </div>
            </div>
            <div className="text-[10px] text-slate-500 mt-2 border-t border-white/[0.04] pt-2 flex justify-between">
              <span>Standard Deduction:</span>
              <span className="font-mono text-slate-400">-{formatINR(standardDeductionOld)}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border border-white/[0.04] rounded-xl p-3.5 flex flex-col justify-between">
            <div>
              <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">New Tax Regime</span>
            ...
            <div className="text-[10px] text-slate-500 mt-2 border-t border-white/[0.04] pt-2 flex justify-between">
              <span>Standard Deduction:</span>
              <span className="font-mono text-emerald-350">-{formatINR(standardDeductionNew)}</span>
            </div>
          </div>
        </div>
        
        <p className="text-[9px] text-slate-550 mt-3 text-center leading-normal">
          Net Taxable Salary = Gross Salary - Standard Deduction. Adjusting the inputs above will immediately update these values.
        </p>
      </div>

      {/* Action Footer */}
      <div className="flex flex-col sm:flex-row items-center justify-end gap-3 border-t border-white/[0.06] pt-5 mt-6">
        <button
          id="btn-cancel-extraction"
          onClick={onCancel}
          type="button"
          className="w-full sm:w-auto px-5 py-2.5 text-xs font-semibold text-slate-400 hover:text-white hover:bg-white/[0.02] rounded-xl transition-colors text-center active:scale-98 cursor-pointer"
        >
          Cancel
        </button>
        <button
