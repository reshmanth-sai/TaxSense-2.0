...
const SavingsChart: React.FC<{ data: { year: string; savings: number; regime: string }[] }> = React.memo(({ data }) => {
  ...
  return (
    <div className="bg-[#0E131B] border border-white/[0.04] rounded-3xl p-6 text-left relative overflow-hidden group">
      ...
      <div className="flex justify-between items-center mb-4">
        <div>
          ...
          <h4 className="text-sm font-bold text-slate-100 mt-0.5">Savings Over Time</h4>
        </div>
        <div className="flex items-center gap-1.5 text-[9px] font-bold text-emerald-450 uppercase tracking-wider bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full select-none">
      ...
      <div className="relative w-full overflow-x-auto">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto min-w-[450px]">
          ...
          {/* Y Axis Line */}
          <line x1={paddingLeft} y1={paddingTop} x2={paddingLeft} y2={height - paddingBottom} stroke="rgba(255,255,255,0.05)" />
        ...
        <AnimatePresence>
          {hoveredPoint !== null && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 5 }}
              className="absolute top-12 left-1/2 -translate-x-1/2 bg-slate-950 border border-white/[0.08] backdrop-blur-md px-3.5 py-2 rounded-xl text-[10px] text-slate-300 font-bold space-y-0.5 pointer-events-none z-10 shadow-2xl"
            >
              <div className="text-slate-500 uppercase tracking-widest text-[8px] font-black">AY {data[hoveredPoint].year} savings</div>
              <div className="font-mono text-emerald-400 font-black text-xs">{formatINR(data[hoveredPoint].savings)}</div>
              <div className="text-slate-400 uppercase text-[8px]">Regime: {data[hoveredPoint].regime}</div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
...
export const HistoryArchive: React.FC<HistoryArchiveProps> = React.memo(({ setActiveStep }) => {
  ...
  return (
    <div className="space-y-8 max-w-[1200px] mx-auto text-slate-100 font-sans py-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/[0.03] pb-6">
        <div className="text-left space-y-1">
          <h1 className="text-xl md:text-2xl font-bold tracking-tight text-slate-100">Your Tax History</h1>
          <p className="text-xs md:text-sm text-slate-405 leading-relaxed font-medium">
        ...
        <div className="flex flex-wrap items-center gap-2 select-none">
          ...
          <button
            ...
            className="h-10 px-3 bg-slate-900 hover:bg-slate-800 border border-white/[0.06] text-slate-300 font-bold text-xs uppercase tracking-wider rounded-xl cursor-pointer transition-all active:scale-98"
          >
      ...
      {activeHistory.length === 0 ? (
        <div className="max-w-lg mx-auto bg-[#0E131B] border border-white/[0.04] rounded-3xl p-10 text-center space-y-6 select-none my-12 shadow-2xl">
          <div className="w-16 h-16 bg-slate-900 border border-white/[0.04] text-slate-500 rounded-full flex items-center justify-center mx-auto">
            <History className="h-7 w-7" />
          </div>
          <div className="space-y-1">
            <h3 className="text-base font-bold text-slate-100">No filings yet</h3>
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            <button
              ...
              className="w-full sm:w-auto h-11 px-5 border border-slate-850 hover:bg-white/[0.02] text-slate-300 font-bold text-xs uppercase tracking-wider rounded-xl cursor-pointer transition-all active:scale-98"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div key={stat.title} className="bg-slate-900/20 border border-white/[0.03] rounded-2xl p-4.5 space-y-1 shadow-sm backdrop-blur-md">
                  <span className="text-[9px] text-slate-500 uppercase tracking-widest font-bold block">{stat.title}</span>
                  <span className="text-sm md:text-base font-black text-slate-100 font-mono tracking-tight block">{stat.val}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/[0.04] pb-3 select-none">
                <div className="flex items-center gap-4">
                  <h3 className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Filing Logs</h3>
                  <button
                    onClick={() => setIsTimelineView(!isTimelineView)}
                    className="text-[9.5px] font-bold uppercase text-blue-400 hover:text-blue-300 transition-colors"
                  >
                ...
                <div className="flex items-center gap-2 relative">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-8 pr-3 py-1 bg-slate-950 border border-white/[0.06] rounded-lg text-xs font-medium text-slate-205 focus:outline-none focus:border-blue-500 w-44"
                    />
                  </div>
                </div>
              </div>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 text-[9.5px] font-bold uppercase tracking-wider rounded-lg border transition-all cursor-pointer ${
                        isActive 
                          ...
                          : 'bg-transparent text-slate-450 border-white/[0.04] hover:text-slate-300'
                      }`}
                    >
                      {chip}
                    </button>
              ...
              {isTimelineView ? (
                // TIMELINE VIEW
                <div className="relative pl-6 space-y-6 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-850 before:border-dashed">
                  {filteredHistory.map((item) => (
                    <div key={item.id} className="relative space-y-2 group">
                      <div className="absolute -left-[20px] top-1.5 w-3 h-3 rounded-full bg-emerald-500 border border-slate-950 ring-4 ring-emerald-500/15 group-hover:scale-110 transition-transform" />
                      
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold text-slate-100 text-xs font-mono">{item.id.replace("Filing ", "")}</span>
                          <span className="text-[9px] bg-blue-600/10 text-blue-400 px-1.5 py-0.5 rounded font-black tracking-wider uppercase">{item.recommendedRegime}</span>
                        </div>
                        <span className="text-[10px] text-slate-500 font-bold">{item.date}</span>
                      </div>
                      
                      <div className="p-4 bg-slate-900/30 border border-white/[0.03] rounded-2xl flex items-center justify-between text-xs hover:border-white/[0.06] transition-all">
                        <div>
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-slate-950 border border-white/[0.04] hover:bg-slate-900 text-slate-300 text-[10px] font-bold uppercase rounded-lg transition-colors cursor-pointer"
                        >
                          Open Document
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredHistory.map((item) => (
                    <div key={item.id} className="bg-[#0E131B] border border-white/[0.04] rounded-2xl p-5 flex flex-col justify-between hover:border-white/[0.08] hover:-translate-y-0.5 transition-all duration-200">
                      <div className="space-y-1">
                        <div className="flex justify-between items-start">
                          <span className="font-extrabold text-slate-205 font-mono text-xs">{item.id}</span>
                          <span className="text-[8.5px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                            {item.status || 'Filed'}
                          </span>
                        </div>
                        <div className="text-[10px] text-slate-450 font-bold">{item.date} • {item.recommendedRegime === 'NEW' ? 'New Regime' : 'Old Regime'}</div>
                        <div className="text-xs text-slate-400 font-medium pt-2 leading-relaxed">
                          Tax optimized and generated successfully. Total estimated tax savings: <strong className="text-emerald-400 font-mono">{formatINR(item.savings || 38000)}</strong>
                        </div>
                      </div>

                      <div className="pt-4 mt-4 border-t border-white/[0.02] flex items-center justify-between select-none">
                        <button
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="p-1.5 bg-slate-900 border border-white/[0.04] hover:bg-slate-800 rounded-lg text-slate-400 hover:text-slate-200 transition-all cursor-pointer"
                          >
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            {/* AI Insights Panel */}
            <div className="bg-slate-905 border border-white/[0.04] rounded-3xl p-6 backdrop-blur-md space-y-4">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block border-b border-white/[0.02] pb-2.5">AI Tax Insights</span>
              
              <div className="space-y-3.5 text-xs text-slate-400 font-semibold">
                <div className="flex justify-between items-center py-1">
                  <span className="text-slate-500">Highest Savings:</span>
                  <span className="font-mono text-emerald-400 font-extrabold">AY 2026-27 (₹51,480)</span>
                </div>
                <div className="flex justify-between items-center py-1 border-t border-white/[0.02] pt-2">
                  <span className="text-slate-500">Most Improved Year:</span>
                  <span className="font-bold text-slate-200">+42%</span>
                </div>
                <div className="flex justify-between items-center py-1 border-t border-white/[0.02] pt-2">
                  <span className="text-slate-500">Average Savings:</span>
                  <span className="font-mono text-slate-200">{formatINR(37200)}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-t border-white/[0.02] pt-2">
                  <span className="text-slate-500">Regime recommendation:</span>
                  <span className="font-bold text-slate-200">New Regime (3 years)</span>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-white/[0.02]">
                <button
                  ...
                  className="w-full py-2.5 bg-slate-950 border border-white/[0.04] hover:bg-slate-900 text-blue-400 font-bold text-[10px] uppercase tracking-wider rounded-xl cursor-pointer transition-all active:scale-95 text-center flex items-center justify-center gap-1.5"
                >
            ...
            <div className="bg-slate-905 border border-white/[0.04] rounded-3xl p-6 backdrop-blur-md space-y-4 select-none">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block border-b border-white/[0.02] pb-2.5">Archived Documents</span>
              ...
              <div className="space-y-3 font-semibold text-xs text-slate-300">
                {[
                  ...
                  <div key={doc.name} className="flex justify-between items-center p-3 bg-slate-950/40 border border-white/[0.02] rounded-xl hover:border-white/[0.04] transition-colors">
                    <div>
                      <div className="text-[11px] font-bold text-slate-200">{doc.name}</div>
                      <div className="text-[9px] text-slate-500">{doc.format}</div>
                    </div>
                    <button
                      onClick={() => handleDownloadMock(doc.name)}
                      className="p-1.5 bg-slate-900 border border-white/[0.04] hover:bg-slate-800 rounded-lg text-slate-400 hover:text-slate-200 transition-colors cursor-pointer"
                    >
                      <Download className="h-3.5 w-3.5" />
                    </button>
                  </div>
            ...
            <div className="bg-slate-905 border border-white/[0.04] rounded-3xl p-6 backdrop-blur-md space-y-4">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block border-b border-white/[0.02] pb-2.5">Protected Archive</span>
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center pb-4 border-b border-white/[0.04]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-slate-100">Exemption Slabs Comparison</span>
                  </div>
                  <button
                    onClick={() => setIsComparisonOpen(false)}
                ...
                <div className="pt-2">
                  <div className="overflow-x-auto border border-white/[0.04] rounded-2xl bg-slate-950/40">
                    <table className="w-full text-left text-xs font-semibold">
                      <thead>
                        <tr className="border-b border-white/[0.04] text-slate-500 uppercase tracking-wider text-[9px]">
                      ...
                      <tbody className="text-slate-300 font-mono">
                        <tr className="border-b border-white/[0.02]">
                          <td className="p-4 font-bold text-slate-100">AY 2026-27</td>
                          <td className="p-4">₹14.5L</td>
                          <td className="p-4 text-blue-400">NEW</td>
                          <td className="p-4 text-emerald-450">₹51,480</td>
                        </tr>
                        <tr className="border-b border-white/[0.02]">
                          <td className="p-4 font-bold text-slate-100">AY 2025-26</td>
                          <td className="p-4">₹11.8L</td>
                          <td className="p-4 text-blue-400">NEW</td>
                          <td className="p-4 text-emerald-450">₹46,000</td>
                        </tr>
                        <tr className="border-b border-white/[0.02]">
                          <td className="p-4 font-bold text-slate-100">AY 2024-25</td>
                          <td className="p-4">₹9.2L</td>
                          <td className="p-4 text-blue-400">NEW</td>
                          <td className="p-4 text-emerald-450">₹32,000</td>
                        </tr>
                        <tr>
                          <td className="p-4 font-bold text-slate-100">AY 2023-24</td>
                          <td className="p-4">₹7.5L</td>
                          <td className="p-4 text-amber-500">OLD</td>
                          <td className="p-4 text-emerald-450">₹18,500</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div className="p-4 bg-blue-500/[0.02] border border-blue-500/10 rounded-2xl flex items-start gap-2.5 text-xs text-slate-400 leading-relaxed mt-6">
                    <Sparkles className="w-4.5 h-4.5 text-blue-400 shrink-0 mt-0.5" />
                    <div className="space-y-1 font-semibold">
                      <span className="text-blue-400 block text-[10px] uppercase tracking-wider">AI Comparative Audit</span>
                      <p>
